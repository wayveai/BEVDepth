...
def parse_args():
    ...
    parser.add_argument(
        "target_path", help="Target path to save the visualization result."
    )
...
def get_3d_lines(corners):
    ...
    for st, ed in [
        ...
        if corners[st, -1] > 0 and corners[ed, -1] > 0:
            ret.append(
                [[corners[st, 0], corners[ed, 0]], [corners[st, 1], corners[ed, 1]]]
            )
    return ret
...
def demo(
    ...
    if infos is None:
        infos = mmengine.load(os.path.join(nusc_root, "nuscenes_infos_val.pkl"))
    assert idx < len(infos)
    # Get data from dataset
    results = mmengine.load(nusc_results_file)["results"]
    info = infos[idx]
    lidar_path = info["lidar_infos"]["LIDAR_TOP"]["filename"]
    lidar_points = np.fromfile(
        os.path.join(nusc_root, lidar_path), dtype=np.float32, count=-1
    ).reshape(-1, 5)[..., :4]
    lidar_calibrated_sensor = info["lidar_infos"]["LIDAR_TOP"]["calibrated_sensor"]
    # Get point cloud
    pts = lidar_points.copy()
    ego2global_rotation = np.mean(
        [info["cam_infos"][cam]["ego_pose"]["rotation"] for cam in IMG_KEYS], 0
    )
    ego2global_translation = np.mean(
        [info["cam_infos"][cam]["ego_pose"]["translation"] for cam in IMG_KEYS], 0
    )
    ...
    for i in range(len(info["ann_infos"])):
        if (
            map_name_from_general_to_detection[info["ann_infos"][i]["category_name"]]
            in show_classes
        ):
            box = get_ego_box(
                dict(
    ...
    for box in results[info["sample_token"]]:
        if (
            box["detection_score"] >= threshold
            and box["detection_name"] in show_classes
        ):
    ...
    for i, k in enumerate(IMG_KEYS):
        ...
        img = mmcv.imread(os.path.join(nusc_root, info["cam_infos"][k]["filename"]))
        ...
        for corners, cls in zip(pred_corners, pred_class):
            ...
            for line in lines:
                plt.plot(
                    line[0], line[1], c=cm.get_cmap("tab10")(show_classes.index(cls))
                )
