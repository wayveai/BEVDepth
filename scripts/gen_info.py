            ...
            for k, lidar_data in enumerate(lidar_datas):
                ...
                for j in range(max_lidar_sweeps):
                    if sweep_lidar_data["prev"] == "":
                        ...
                        sweep_lidar_data = nusc.get(
                            "sample_data", sweep_lidar_data["prev"]
                        )
                        sweep_lidar_info = dict()
                        ...
                        if (
                            sweep_lidar_info["sample_token"]
                            != lidar_data["sample_token"]
                        ):
                            break
                        sweep_lidar_info["ego_pose"] = nusc.get(
...
def main():
    ...
    mini_trainval_nusc = NuScenes(
        version="v1.0-mini", dataroot="./data/nuScenes/", verbose=True
    )
    mini_train_scenes = splits.mini_train
    mini_val_scenes = splits.mini_val
    mini_train_infos = generate_info(mini_trainval_nusc, mini_train_scenes)
    mini_val_infos = generate_info(mini_trainval_nusc, mini_val_scenes)
    mmengine.dump(mini_train_infos, "./data/nuScenes/nuscenes_infos_mini_train.pkl")
    mmengine.dump(mini_val_infos, "./data/nuScenes/nuscenes_infos_mini_val.pkl")
